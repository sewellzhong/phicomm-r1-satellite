...
```bash
--es playback_reference_path <diagnostics内参考文件的绝对路径>
```

路径仍由应用按规范路径强制限制在自身`diagnostics`目录内；只传文件名会被拒绝。采集固定
保留1秒前导和至少1秒尾段，并记录设备实际媒体音量index/max，而不相信命令行
...
设备的播放耗时与起止点来自不同系统时钟；审计只允许前者比单调时钟差值最多多10 ms，
用于覆盖实机已观察到的约1.5 ms量化/时钟偏差，超出仍拒绝。
