...
- 3448取证调用固定为 `CLASSPATH=/system/framework/pm.jar app_process /system/bin com.android.commands.pm.Pm install -r <candidate>`，显式降级只增加固定 `-d`。退出码必须为0；标准输出只接受单行`Success`，或路径逐字等于本次固定远端APK的`pkg: <路径>`后紧跟`Success`。随后仍须通过 `dumpsys package` 与设备端实际 APK 哈希独立观察目标版本，不能只信命令返回。
...
首次真实计划已消费并保留失败证据：3448实际输出固定为`pkg: <路径>`加`Success`，旧解析器因此在成功观察v118后进入回退，并在设备实际恢复v102后仍保守记为失败。设备随后独立核对为精确v102、Enforcing、监听和隔离均恢复。解析修正及新计划边界见[实机记录](2026-09-13-r1-update-package-manager-device.md)。下一步只有在修正通过完整门禁并提交后，才消费基于新v6只读证据封存的第二个计划；执行结果仍只能用于选择固定argv或Binder后端及最小权限策略，不能直接当作独立监督器自动回滚通过。
