...
| v76 每轮扫描快照 | 待提交；实机前填写完整 SHA | API 22 / Java 8、Android 153 项及 lint 通过 | 待验证 | 开窗前 station 模式扫描并把去重、去 BSSID 的结果只保留在内存；AP 期间刷新读取本轮快照；关窗、恢复或提交后清除 | 固件无法在 SoftAP 模式实时扫描，因此页面文案不再承诺重新扫描；新目标热点需在下一轮开窗前保持可发现 |
| v77 异步 Wi-Fi 恢复重试 | 待提交；实机前填写完整 SHA | API 22 / Java 8、Android 153 项、lint、工具 32 项通过；完整检查待提交后执行 | 待验证 | SoftAP 关闭后后台每 500 ms 重试 Wi-Fi 启用，最长 20 秒；真实启用才清 pending，失败保留标记供下次服务启动恢复 | v75 自动关窗后 AP 已消失但原地址、mDNS 均不可达，推断即时 `setWifiEnabled(true)` 被 AP 异步关闭状态拒绝；当前需整机重启 |
