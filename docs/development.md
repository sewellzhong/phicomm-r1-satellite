...
主机套件包含 132 项 Android、30 项部署/诊断工具和 44 项 HA 测试，以及 HA 配置加载检查。另两项既有 HA 私有路由源码测试位于 `tools/ha/local_tests/`，家中准备原 `local-deps/ha-live-routing-2026-09-06/` 后显式执行；不计入公开套件，不以模拟替身替换其真实源码结论。
...
4. 主机检查通过后可合入 `main`，硬件相关功能保持待验证。换电脑前提交并推送，另一台电脑拉取相同分支；不通过复制整个工作目录同步。

不依赖设备的工作可完成。依赖尚未知硬件能力的部分注明假设与阻塞，继续其他可独立开发内容，不将未知能力当成已具备。

## 回家验证

先保存当前工作，再 fetch 并检出待验证的确切提交。使用家中原有签名和 `local-deps/private-prompts/assets/`，省略 `-PhostCheck=true` 构建原独立包名。公开 clone 默认缺少中文素材，此构建会明确失败；原素材经校验保留在家中。
...
安装前核对 API 22 / 固件 3448、已有签名、原厂备份与回退 APK；使用既有部署工具。保留家中 Android debug keystore，禁止共享到 Git/CI；签名不匹配时停止，不卸载应用绕过。hostcheck 包会被部署工具拒绝。
