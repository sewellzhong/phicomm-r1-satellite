...
准备流程需要互联网；Gitleaks 8.30.1、TFLite AAR、protobuf 与测试 JAR 均按固定 SHA-256 下载，检查只扫描公开文件，不读取被忽略的私有资料。HA 测试容器禁网，不连接实际设备服务。SDK/依赖下载失败或校验失败会中止。Gradle wrapper 固定 8.9，协议/TFLite/原生源码版本不随环境自动升级。依赖在 `local-deps/` 重建，CMake 每次重新配置当前 SDK/NDK 路径，不复制私有密钥。
...
主机套件包含 163 项 Android、32 项部署/诊断工具、18 项 R0 恢复工具、R0 桌面演练、7 项原厂音频代理 socket 契约测试（包括断连释放，以及 1,000/90,000 帧的 20 秒及 30 分钟音频预算加速演练）和 44 项 HA 测试，以及 lint、hostcheck APK、API 22 ARMv7 代理交叉构建和 HA 配置加载检查。首次在线检查会准备 Gradle/AGP 自身依赖；相同版本已缓存且 `prepare.py` 已成功后，可用 `R1_GRADLE_OFFLINE=1 bash tools/dev/check.sh` 排除远端仓库波动。另两项既有 HA 私有路由源码测试位于 `tools/ha/local_tests/`，本地准备原 `local-deps/ha-live-routing-2026-09-06/` 后显式执行；不计入公开套件，不以模拟替身替换其真实源码结论。
...
后续任务按[优化分析 O1～O8](2026-09-07-r1-chain-optimization.md)关联到已有功能组：O1 随功能记录，O2 为播放基础，O3/O5/O6 协同流式与 HA 接入，O4 随双入口管理，O8 依据测量评估；O7 留在最终语音回归及修复闭环。硬件/配网/蓝牙仍是当前入口，无设备时可继续独立开发。
