...
主机套件包含 146 项 Android、32 项部署/诊断工具和 44 项 HA 测试，以及 HA 配置加载检查。首次在线检查会准备 Gradle/AGP 自身依赖；相同版本已缓存且 `prepare.py` 已成功后，可用 `R1_GRADLE_OFFLINE=1 bash tools/dev/check.sh` 排除远端仓库波动。另两项既有 HA 私有路由源码测试位于 `tools/ha/local_tests/`，本地准备原 `local-deps/ha-live-routing-2026-09-06/` 后显式执行；不计入公开套件，不以模拟替身替换其真实源码结论。
