...
当前状态与开发顺序见[根 README](../README.md)。历史报告保持原结论；旧下一步不作为执行指令。2026-09-16 已确认此前部分测试中麦克风朝下，方向、距离、声学效果和主观听感须在正确姿态下重新真机验收；当前执行边界见[姿态纠正与真机验收恢复决定](2026-09-16-r1-repositioned-microphone-revalidation-plan.md)和[现行真人功能计划](2026-09-14-r1-human-voice-multi-device-plan.md)。
...
- [真人功能验收与多台部署现行计划](2026-09-14-r1-human-voice-multi-device-plan.md)：先完成 `r1-sample01` 单台 release gate，再逐台建立其他设备的独立身份、基线、HA注册和回滚证据；全部单机通过后才做 3～8 台并发回归。
- [v126计时器/普通媒体所有权实机记录](2026-09-14-r1-v126-timer-media-ownership-device.md)：真实HA中文计时器两轮自然到期均暂停媒体；系统暂停可条件恢复，用户停止后不复活，最终清理与监听恢复通过。
- [v128本地闹钟音乐与自定义计时提示实机记录](2026-09-14-r1-v128-local-alert-audio-device.md)：v127首次上传失败后，v128/HA0.16.1以4 KiB认证分块通过真实上传、哈希回读、计时器/闹钟自然到时本地音频、服务重启保持、引用删除保护和最终清理。
- [本地闹钟音乐与自定义计时提示主机实现](2026-09-14-r1-local-alert-audio-host.md)：认证分块、本地哈希/WAV校验、离线播放、闹钟逐项音乐和计时器自定义提示内容的设计与主机边界。

- [原厂方案与功能/数据/链路验证边界](2026-09-12-r1-vendor-native-functional-validation.md)：2026-09-16已因麦克风朝下事实恢复正确姿态下的真人、方向、距离、声学和体感真机验收；历史姿态错误结果保留并标记需重测。
- [v104本地计时器主机实现](2026-09-12-r1-native-timers-host.md)：标准事件、本地持久倒计时、暂停/重启时钟边界、断网铃声、停止、抢占及重放去重已通过主机门禁，首台实机待验证。
...
- [原厂四麦与音频调校优先路线](2026-09-08-r1-factory-audio-root-plan.md)：原厂链、完整回刷、Root/SELinux/特权代理和降级边界；其中旧真人声学验收已由2026-09-12现行验证边界取代。
...
- [2026-09-16 r1-sample01 播放中唤醒阈值 229 对照测试](2026-09-16-r1-playback-wake-threshold-229.md)
...
- [2026-09-12-r1-local-alarm-core-host](2026-09-12-r1-local-alarm-core-host.md)
- [2026-09-12-r1-live-router-streaming-implementation](2026-09-12-r1-live-router-streaming-implementation.md)
