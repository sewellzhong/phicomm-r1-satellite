...
- 当前开发源码为 v71（配网页面窗口重建），首台设备已部署 v70；现场盘点见[硬件能力探测](docs/2026-09-07-r1-hardware-capability.md)。安卓与 iPhone 的 A2DP 配对、播放、断开和重连播放通过。v67 复用原厂 NetControl 后，配网灯、热点、网页服务和 Wi-Fi 扫描 API 已通过实测，但原固件首页文件缺失且无提示音。v69 的优化页面和扫描列表实机通过；首次提交只发起连接，R1 未连上临时热点并回退原网络。v70 增加目标连接重试，但提前退出后再次开窗会被旧定时器关闭页面；v71 每轮完整重建页面服务并取消旧定时器，待复验。
