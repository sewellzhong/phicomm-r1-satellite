...
- 当前开发源码为 v77（异步 Wi-Fi 恢复重试），首台设备已部署 v75；现场盘点见[硬件能力探测](docs/2026-09-07-r1-hardware-capability.md)。安卓与 iPhone 的 A2DP 配对、播放、断开和重连播放通过。v75 曾成功恢复遗留 SoftAP，系统消息中心按键接收器注册成功；双浏览器检查确认第二个终端被 409 拒绝。该轮关闭后 SoftAP 消失但 Wi-Fi 客户端未重新启用，当前设备需整机重启。v77 已加入每轮扫描快照和后台 Wi-Fi 启用重试，只有客户端真实启用后才清除 pending，待设备恢复后部署。
