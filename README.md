...
- 当前开发源码为 v68（自助配网页面），首台设备已部署 v67；现场盘点见[硬件能力探测](docs/2026-09-07-r1-hardware-capability.md)，部署与既有验证见[清理报告](docs/2026-09-07-repository-cleanup.md)。v66 实机证实普通 APK 无法从 evdev 或 Activity 收到中央键/音量环，相关无效前台转发已在 v67 移除。安卓与 iPhone 的 A2DP 配对、播放、断开和重连播放通过。v67 复用原厂 NetControl 后，配网灯、热点、网页服务和 Wi-Fi 扫描 API 已通过实测，但原固件首页文件缺失且无提示音。v68 在独立 APK 提供配网页面，待逐项实机验证。
...
2026-09-07 已将[全链路优化 O1～O8](docs/2026-09-07-r1-chain-optimization.md)纳入开发。v64 完成 O1/O2 的首个无设备增量；v65/v66 增加硬件及限时蓝牙、BLE、热点能力探针。手机蓝牙音箱已通过两类手机实测。中央键和音量环的普通 APK 输入路径、LED 写入受权限或框架拦截；自建热点实测失败。v67 接入保留的原厂配网服务，并增加热点状态回读。v68 在 8080 端口补齐缺失的移动端页面，代理原厂扫描 API，并通过 Android Wi-Fi 配置接口提交凭据；应用不记录或持久保存密码。
