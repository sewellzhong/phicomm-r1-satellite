...
- v81（`f7723c945da5dc222b5d058f6265c3492db6e335`）原厂音频代理沿用真实 3448 动态后端、固定 v1 Protobuf IPC、UID/SELinux隔离及生产失败关闭门槛，并为显式限时验证请求增加双输出通道诊断旁路。生产流仍固定为PCM S16LE、16 kHz、单声道、20 ms；诊断另存双通道WAV并审计哈希、通道非零性、差异和单声道映射。该代码尚未部署，不能把主机测试写成运行时通道形状已通过，更不能据此宣称四麦独立响应、AEC或DSP质量。v80实机结果及原厂链历史见[主机开发记录](docs/2026-09-08-r1-factory-audio-agent-host.md)。
...
当前boot-only权限环境、专用代理、原厂四麦阵列PCM及DOA传输门槛已实机通过；继续完成四麦独立响应、通道形状、AEC消除量和DSP质量的R3声学准入。R0恢复门槛并未取消，只是不再作为首台boot实验的前置。之后补齐单台核心功能：播放中打断、流式回答、主动播报与计时器、本地闹钟、免打扰、媒体播放、手机蓝牙音箱、安卓/iPhone 自助配网、按键/物理静音/灯效、设备名称/区域与系统管理、签名更新与回滚。以上为待完成交付的功能组，不表示全部尚无代码。
