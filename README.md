...
- v81（`f7723c945da5dc222b5d058f6265c3492db6e335`）原厂音频代理沿用真实 3448 动态后端、固定 v1 Protobuf IPC、UID/SELinux隔离及生产失败关闭门槛，并为显式限时验证请求增加双输出通道诊断旁路。生产流仍固定为PCM S16LE、16 kHz、单声道、20 ms；诊断另存双通道WAV并审计哈希、通道非零性、差异和单声道映射。`9ad9e6c873c876d1d44ce05122fde81d0aaa4e58`又增加四方向DOA矩阵审计，在未知设备零度下核对有效帧比例、集中度和相对方向；其`pass`只证明上报DOA的相对响应。v82源码（`519def937b28b095dbfd0e63ea17cb7e6ed30e9e`）允许在最长30秒验证窗口内延迟1秒并发播放受控本地参考，记录参考哈希、真实媒体音量档位及播放单调时钟边界；离线审计缺少或篡改任一旁证即拒绝。2026-09-11已在提交`76a2b4fba029a50d6bc090cae93e8cd691ff9b36`完成统一主机检查并生成同签名v82设备候选，APK SHA-256为`b115b5077da502a7fbe9efd8a21e2cee6c2e5ae084e8c176d8ace79d52db1c73`；v82重建的代理仍为v81基线`d06325edb1b048bec990c822a015986b762a27263cc8d5ffd0e9a7ad337e5843`，但设备当前v13 boot内仍为v80代理`4a2e7e8d4be302cd4e87bac88a838bccdc2f1058cccaa20762e0202133e13be0`，所以仍须受控更新boot代理。v81/v82仍未部署，不能把主机测试写成运行时通道形状、播放参考覆盖、AEC、四麦独立响应或DSP质量已通过。v80实机结果及原厂链历史见[主机开发记录](docs/2026-09-08-r1-factory-audio-agent-host.md)。
...
当前boot-only权限环境、专用代理、原厂四麦阵列PCM及DOA传输门槛已实机通过；v81通道形状采集、四方向DOA离线矩阵审计及v82受控播放同步采集已具备，同签名v82设备候选也已生成并核对，等待受控实机输入。现已增加独立的boot代理增量构建器，严格绑定当前boot双读、v13候选/overlay清单与新旧代理哈希，并核对除代理外ramdisk条目及所有不可变boot组件不变；它不接触设备或执行写入。下一步现场新读两份当前boot后生成只替换代理的固定候选，并按身份/范围核对、单次写入、复位前完整读回门禁部署v81代理；启动后复核Android、ADB、SELinux Enforcing、网络和音频，再覆盖安装v82 APK。随后按相同参考和实际媒体音量档位取得安静、60%和90%播放样本。这些采集只建立AEC定量输入，不直接证明AEC。随后继续完成四麦独立响应、AEC消除量和DSP质量的R3声学准入。R0恢复门槛并未取消，只是不再作为首台boot实验的前置。之后补齐单台核心功能：播放中打断、流式回答、主动播报与计时器、本地闹钟、免打扰、媒体播放、手机蓝牙音箱、安卓/iPhone 自助配网、按键/物理静音/灯效、设备名称/区域与系统管理、签名更新与回滚。以上为待完成交付的功能组，不表示全部尚无代码。
