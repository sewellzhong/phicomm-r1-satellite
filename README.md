...
- v28/v88 主机增量已按调用入口归属诊断窗口，修复“关闭时进入、开启后返回”被误计为`invalid`的竞态；原`trylock`队列改为32槽SPSC环形队列，并把第二生产线程、队列满及三类真实ABI异常独立上报。factory-audio v1只追加兼容字段，schema 1历史材料仍可审计，schema 2严格要求完整性明细。当前仅为代码与模拟测试支持，尚未更新设备boot、安装v88或重复四方向矩阵，R3/R0不变。
