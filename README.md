...
2026-09-07 已将[全链路优化 O1～O8](docs/2026-09-07-r1-chain-optimization.md)纳入开发。v64 完成 O1/O2 的首个无设备增量；v65/v66 增加硬件及限时蓝牙、BLE、热点能力探针。手机蓝牙音箱已通过两类手机实测。中央键和音量环的普通 APK 输入路径、LED 写入受权限或框架拦截；中央键系统消息桥现已把短按接入统一取消。v67 接入保留的原厂配网服务，v68～v77 补齐移动端页面、扫描快照及恢复状态机。v78 允许最多 16 个独立浏览器会话同时浏览和填写，只在最终提交时仲裁，并强制页面与接口不缓存。v79 的一次性交接密文由设备 Keystore 保护，成功或失败后删除；应用和日志不保存明文密码。v89源码在回复处理和播放期间继续运行固定Alexa模型，新唤醒先停止本地播放并取消旧会话，旧播放器完全释放后才打开新命令窗口。v90进一步实现无需Alexa的直接插话：持续语音通过VAD/能量门槛后立即取消旧回复，释放期间有界暂存原厂PCM，取得新运行所有权后继续上传；播放参考只处理检测副本，不改写上传PCM或宣称AEC效果。v91开始推进O3：HA会话包装层仅在绑定路由真实支持流式时报告能力，并把隔离内层会话的增量文字透传给Assist流式TTS；非流式和接口缺失路径失败关闭。v92进一步把增量转发限定在单次调用生命周期，合成门闩证明首段先于全文完成，取消后旧聊天日志引用不能再把迟到文字送入外层监听器。以上增量均仅为代码/主机支持，真实TTS首块与播放写入、R1功能、家庭路由、至少60秒边生成边播放及工具确认边界仍待自动链路验证，见[v90主机记录](docs/2026-09-12-r1-direct-barge-in-host.md)、[v91流式会话记录](docs/2026-09-12-r1-streaming-conversation-host.md)和[v92取消隔离记录](docs/2026-09-12-r1-streaming-cancellation-host.md)。
