...
- 当前开发源码为 v64（统一音频取消基础），首台设备仍为 v63 原生链路清理版；部署与验证结果见[清理报告](docs/2026-09-07-repository-cleanup.md)。v64 已在 Temurin JDK 17.0.19 下完成 Java 8 目标编译、138 项 Android 测试（其中 ESPHome/取消/会话/播放器相关 50 项）、lint、hostcheck APK、30 项工具测试和 44 项 HA 测试；R1 实机待验证，v63 的既有结果不自动继承给 v64。
...
2026-09-07 已将[全链路优化 O1～O8](docs/2026-09-07-r1-chain-optimization.md)纳入开发。v64 完成 O1/O2 的首个无设备增量：Assist TTS 取消请求先本地停播和清除队列，再由协议所有者线程发送远端取消；旧会话迟到事件被隔离，新会话等待播放器资源完全释放，并记录有界状态、缓冲和取消事件。按键、新唤醒、直接插话以及其他音频来源尚未接入或实机验收。
