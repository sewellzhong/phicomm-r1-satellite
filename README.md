...
- 当前开发源码为 v76（每轮扫描快照），首台设备已部署 v75；现场盘点见[硬件能力探测](docs/2026-09-07-r1-hardware-capability.md)。安卓与 iPhone 的 A2DP 配对、播放、断开和重连播放通过。v75 已通过热点 ADB 安装并成功关闭遗留 SoftAP，R1 回到原网络及 `listening`，系统消息中心按键接收器注册成功。双浏览器自动检查确认第二个终端被 409 拒绝；同时发现固件进入 SoftAP 后清空扫描结果。v76 在开热点前保存本轮内存扫描快照，关窗立即清除，待部署复验。
