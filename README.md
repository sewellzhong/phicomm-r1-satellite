...
- 当前开发源码为 v66（Android 事件分发硬件输入），首台设备已部署 v65 能力探针；现场盘点见[硬件能力探测](docs/2026-09-07-r1-hardware-capability.md)，部署与既有验证见[清理报告](docs/2026-09-07-repository-cleanup.md)。v65 的卫星运行、蓝牙/BLE/热点能力查询通过，但后台 evdev 被 SELinux 拒绝；v66 改用前台 Activity 向常驻服务转发，已完成 API 22 / Java 8 编译、148 项 Android 测试、lint、hostcheck APK 和 32 项工具测试，R1 待复验。
...
2026-09-07 已将[全链路优化 O1～O8](docs/2026-09-07-r1-chain-optimization.md)纳入开发。v64 完成 O1/O2 的首个无设备增量；v65/v66 增加中央键/音量环探针、逻辑音量同步及限时蓝牙可发现、BLE GATT、热点网页能力窗口。中央键尚未绑定停止等产品动作，灯光普通 APK 写入、手机配对/播放、BLE 与热点均待实机验证。
