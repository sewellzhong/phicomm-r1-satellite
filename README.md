...
- 当前开发源码为 v65（硬件与配网/蓝牙能力探针），首台设备仍为 v63 原生链路清理版；现场盘点见[硬件能力探测](docs/2026-09-07-r1-hardware-capability.md)，部署与既有验证见[清理报告](docs/2026-09-07-repository-cleanup.md)。v65 已在 Temurin JDK 17.0.19 下完成 API 22 / Java 8 编译、146 项 Android 测试、lint、hostcheck APK、32 项工具测试、44 项 HA 测试及配置加载检查；R1 部署待本轮后续验证，v63 的既有结果不自动继承。
...
2026-09-07 已将[全链路优化 O1～O8](docs/2026-09-07-r1-chain-optimization.md)纳入开发。v64 完成 O1/O2 的首个无设备增量；v65 增加后台中央键/音量环探针、逻辑音量同步及限时蓝牙可发现、BLE GATT、热点网页能力窗口。中央键尚未绑定停止等产品动作，灯光普通 APK 写入、手机配对/播放、BLE 与热点均待实机验证。
