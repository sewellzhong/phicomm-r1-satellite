...
- v22 按原厂管理器改用2,400字节HAL读取后，右侧真人发声使原始四麦四通道RMS均约升至静默的3倍，证明原始四麦边界接收到人声；未唤醒态的处理输出和DOA仍无响应。v23在约1秒后切换原厂已唤醒状态，DOA从5°切到115°且ASR/VAD非零，但切换造成2个sidecar缺口和5个代理节拍丢失。v24～v27把切换移到录音前并增加无保存预热；当前v27取得312次连续有效调用、0缺口/0 dropped/0代理丢帧、ASR/VAD非零及115°档DOA，但仍有1次非标准调用，严格审计失败。设备当前运行v27 boot `973e00ed…30df`与v87 APK，Enforcing和监听已恢复；四方向DOA、AEC、DSP质量及R3仍未验证，详见[唤醒状态实机记录](docs/2026-09-12-r1-factory-audio-postwake-device.md)。
