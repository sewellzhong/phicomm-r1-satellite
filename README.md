...
2026-09-07 已将[全链路优化 O1～O8](docs/2026-09-07-r1-chain-optimization.md)纳入开发。v64 完成 O1/O2 的首个无设备增量；v65/v66 增加硬件及限时蓝牙、BLE、热点能力探针。手机蓝牙音箱已通过两类手机实测。中央键和音量环的普通 APK 输入路径、LED 写入受权限或框架拦截；中央键系统消息桥现已把短按接入统一取消。v67 接入保留的原厂配网服务，v68～v77 补齐移动端页面、扫描快照及恢复状态机。v78 允许最多 16 个独立浏览器会话同时浏览和填写，只在最终提交时仲裁，并强制页面与接口不缓存。v79 的一次性交接密文由设备 Keystore 保护，成功或失败后删除；应用和日志不保存明文密码。v89源码在回复处理和播放期间继续运行固定Alexa模型，新唤醒先停止本地播放并取消旧会话，旧播放器完全释放后才打开新命令窗口。v90进一步实现无需Alexa的直接插话：持续语音通过VAD/能量门槛后立即取消旧回复，释放期间有界暂存原厂PCM，取得新运行所有权后继续上传；播放参考只处理检测副本，不改写上传PCM或宣称AEC效果。v91开始推进O3：HA会话包装层仅在绑定路由真实支持流式时报告能力，并把隔离内层会话的增量文字透传给Assist流式TTS；非流式和接口缺失路径失败关闭。v92进一步把增量转发限定在单次调用生命周期，合成门闩证明首段先于全文完成，取消后旧聊天日志引用不能再把迟到文字送入外层监听器。v93使用固定HA 2026.8.2的真实`PipelineInput`、意图流式阈值和TTS message stream接入确定性PCM播放替身，证明首个PCM写入先于全文完成、分段顺序完整，且取消后迟到增量不产生新播放写入。v94进一步以三段各21秒的确定性PCM验证63秒输出顺序、唯一性和完整排空，并用显式工具结果门闩证明结果前仅播放处理中段、结果后才播放成功段。v95核对仓库外固定的2026-09-06家庭路由源码快照，确认该实际路由不声明流式能力，卫星适配层正确失败关闭为非流式；来源绑定会话和控制拒绝回归仍通过。以上增量均仅为代码/主机支持，当前真实后端已确认不满足O3；实际TTS/HTTP/R1播放、真实墙钟至少60秒边生成边播放及结构化工具结果仍待自动链路验证，见[v90主机记录](docs/2026-09-12-r1-direct-barge-in-host.md)、[v91流式会话记录](docs/2026-09-12-r1-streaming-conversation-host.md)、[v92取消隔离记录](docs/2026-09-12-r1-streaming-cancellation-host.md)、[v93 Assist管线记录](docs/2026-09-12-r1-streaming-assist-pipeline-host.md)、[v94长回答记录](docs/2026-09-12-r1-streaming-long-reply-host.md)和[v95实际路由核查](docs/2026-09-12-r1-live-router-streaming-audit.md)。

当前boot权限环境、专用代理、原厂四麦阵列PCM及DOA传输门槛已实机通过；设备当前运行v28代理和v88 APK。v28已排除v27的固定`invalid=1`和旁路队列竞争，五个实机窗口全部满足schema 2完整性门槛。非专业真人定位、声学和主观体验验收按用户决定停止，不再阻塞开发；历史失败/通过只保留为诊断，不继承为当前结论。实现直接使用原厂四麦、MicArray、DOA、AEC、DSP和校准链；后续持续检查原厂调用、数据、状态、协议、安全、恢复和稳定性，不自行重映射或伪造原厂效果阈值。流式回答的卫星适配与主机管线已实现，但v95已确认当前固定家庭路由不支持流式；需先在对应外部项目补齐增量和结构化工具结果契约，再进行真实TTS/HTTP/R1自动链路验证。随后进入主动播报与计时器、本地闹钟、免打扰、媒体播放、安卓/iPhone自助配网、按键/物理静音/灯效、设备名称/区域与系统管理、签名更新与回滚。播放中打断已有主机实现但仍待自动实机验证；手机蓝牙音箱已有历史实机证据。AEC/DOA/DSP效果及真人唤醒/识别/插话体验标记用户跳过/未验收；R0恢复门槛未取消。以上为待完成交付的功能组，不表示全部尚无代码。
