...
当前boot-only权限环境、专用代理、原厂四麦阵列PCM及DOA传输门槛已实机通过；v81通道形状采集、四方向DOA离线矩阵审计及v82受控播放同步采集已具备，同签名v82设备候选也已生成并核对，等待受控实机输入。下一步先用双份一致的当前boot输入生成只替换代理的固定候选，并按身份/范围核对、boot双读、单次写入、复位前完整读回门禁部署v81代理；启动后复核Android、ADB、SELinux Enforcing、网络和音频，再覆盖安装v82 APK。随后按相同参考和实际媒体音量档位取得安静、60%和90%播放样本。这些采集只建立AEC定量输入，不直接证明AEC。随后继续完成四麦独立响应、AEC消除量和DSP质量的R3声学准入。R0恢复门槛并未取消，只是不再作为首台boot实验的前置。之后补齐单台核心功能：播放中打断、流式回答、主动播报与计时器、本地闹钟、免打扰、媒体播放、手机蓝牙音箱、安卓/iPhone 自助配网、按键/物理静音/灯效、设备名称/区域与系统管理、签名更新与回滚。以上为待完成交付的功能组，不表示全部尚无代码。
