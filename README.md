...
- v22 按原厂管理器改用2,400字节HAL读取后，右侧真人发声使原始四麦四通道RMS均约升至静默的3倍，证明原始四麦边界接收到人声；未唤醒态的处理输出和DOA仍无响应。v23在约1秒后切换原厂已唤醒状态，DOA从5°切到115°且ASR/VAD非零，但切换造成2个sidecar缺口和5个代理节拍丢失。v24～v27把切换移到录音前并增加无保存预热。v27四方向真人诊断均有500个有效DOA且处理输出随人声上升，但前/左同落310°档、右/后落130°/140°档，没有形成四个近似90°方向；所有窗口仍有`invalid=1`，前/左另各丢1次旁路调用，正式方向矩阵不具备运行资格。设备当前运行v27 boot `973e00ed…30df`与v87 APK，Enforcing和监听已恢复；AEC、DSP质量及R3仍未验证，详见[唤醒状态实机记录](docs/2026-09-12-r1-factory-audio-postwake-device.md)。
- v28 boot `2499afb281a2c3cfb7c7e47b376501092a94f2dad2aa5f2973717d28ada5ac6f`与v88 APK `b5120e9fb6b2ddcd70d0df592ee9fb249da1fea16a6f18b3a6e82f3b27aa6710`已部署。实时v27 boot双读、仅代理候选、单次写入、复位前完整读回、Android/Enforcing/Wi-Fi/原厂音频预检、APK回读及监听恢复全部通过。v28按调用入口归属诊断窗口，修复“关闭时进入、开启后返回”被误计为`invalid`的竞态；原`trylock`队列改为32槽SPSC环形队列，并把第二生产线程、队列满及三类真实ABI异常独立上报。首轮10秒静默取得500帧、624次连续MicArray调用，所有真实完整性计数为0，schema 2严格审计通过；四方向矩阵尚未重复，R3/R0不变。
...
当前boot权限环境、专用代理、原厂四麦阵列PCM及DOA传输门槛已实机通过；设备当前运行v28代理和v88 APK。v27四方向诊断曾因固定`invalid=1`、前/左各一次队列丢失及方向折叠而失败；v28静默窗口已证明窗口归属和无竞争队列在真机上得到500帧/624次连续调用及全部真实完整性计数0。下一步先做右侧真人完整性检查，通过后重复前、右、后、左正式矩阵。已查明代理原有0参考/false是硬编码的保守验收声明，不是原厂运行时查询；原厂固定配置确实声明双参考和AEC开启，但没有外部PCM注入ABI，也尚未证明本机播放覆盖或消除效果，详见[AEC边界记录](docs/2026-09-11-r1-factory-audio-aec-boundary.md)。v20固定文件路线因只得到0字节文件而停止。AEC和DSP质量继续排在完整方向矩阵之后。R0恢复门槛并未取消，只是不再作为首台boot实验的前置。再补齐单台核心功能：播放中打断、流式回答、主动播报与计时器、本地闹钟、免打扰、媒体播放、手机蓝牙音箱、安卓/iPhone自助配网、按键/物理静音/灯效、设备名称/区域与系统管理、签名更新与回滚。以上为待完成交付的功能组，不表示全部尚无代码。
