...
- R0 主机端清单、双副本复读校验、门槛报告、现场模板和一键合成演练已实现；工具不含设备连接或刷写能力。完整 eMMC 备份、低层恢复入口和受控完整回刷仍须在 `r1-sample01` 实机完成，当前保持“待验证”。操作边界见 [R0 恢复手册](docs/2026-09-08-r1-r0-recovery-runbook.md)。
- v80 开始实现不依赖设备的原厂音频代理骨架：固定 v1 Protobuf IPC、Unix socket、`SO_PEERCRED` UID 校验、单采集所有者、播放参考、健康状态、显式错误和 APK 客户端已进入主机测试。当前代理只有显式启用的合成假后端，不包含或调用原厂库；真实四麦、DOA、AEC和DSP仍全部“待实机验证”，详见[主机开发记录](docs/2026-09-08-r1-factory-audio-agent-host.md)。
