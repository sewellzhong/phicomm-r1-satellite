...
当前boot权限环境、专用代理、原厂四麦阵列PCM及DOA传输门槛已实机通过；设备当前运行v28代理和v102 APK。v28已排除v27的固定`invalid=1`和旁路队列竞争，五个实机窗口全部满足schema 2完整性门槛。非专业真人定位、声学和主观体验验收按用户决定停止，不再阻塞开发；历史失败/通过只保留为诊断，不继承为当前结论。实现直接使用原厂四麦、MicArray、DOA、AEC、DSP和校准链；后续持续检查原厂调用、数据、状态、协议、安全、恢复和稳定性，不自行重映射或伪造原厂效果阈值。流式回答、真实长流播放及协议驱动的播放中取消已在首台通过；当前下一项是主动播报与计时器，随后为本地闹钟、免打扰、媒体播放、安卓/iPhone自助配网、按键/物理静音/灯效、设备名称/区域与系统管理、签名更新与回滚。手机蓝牙音箱已有历史实机证据。AEC/DOA/DSP效果及真人唤醒/识别/插话体验标记用户跳过/未验收；R0恢复门槛未取消。以上为待完成交付的功能组，不表示全部尚无代码。
