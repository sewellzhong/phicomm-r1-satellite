...
- 2026-09-10 用户进一步决定跳过拆机和物理Maskrom，仅对 `r1-sample01` 接受无可靠恢复入口时修改boot可能永久变砖或丢失数据的风险。R0继续为`pending`；豁免只覆盖boot，不覆盖system、recovery、Loader、分区表、擦除或其他设备。仓库现有设备/原boot哈希绑定的风险门禁、专用enforcing策略补丁、确定性boot构建和验证；私有v2候选从原boot A/B分别构建并逐字节一致，SHA-256为`aaa5b96686a89bd025b752ce6be1e038565c0ec2d6457707576ca419c5f3131c`，实机写入及启动结果见设备清单。
...
当前按设备限定风险豁免先完成boot-only权限环境、特权代理和四麦/AEC/DSP声学准入；R0恢复门槛并未取消，只是不再作为首台boot实验的前置。随后补齐单台核心功能：播放中打断、流式回答、主动播报与计时器、本地闹钟、免打扰、媒体播放、手机蓝牙音箱、安卓/iPhone 自助配网、按键/物理静音/灯效、设备名称/区域与系统管理、签名更新与回滚。以上为待完成交付的功能组，不表示全部尚无代码。
