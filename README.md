...
2026-09-07 已将[全链路优化 O1～O8](docs/2026-09-07-r1-chain-optimization.md)纳入开发。v64 完成 O1/O2 的首个无设备增量；v65/v66 增加硬件及限时蓝牙、BLE、热点能力探针。手机蓝牙音箱已通过两类手机实测。中央键和音量环的普通 APK 输入路径、LED 写入受权限或框架拦截；中央键系统消息桥现已把短按接入统一取消。v67 接入保留的原厂配网服务，v68～v77 补齐移动端页面、扫描快照及恢复状态机。v78 允许最多 16 个独立浏览器会话同时浏览和填写，只在最终提交时仲裁，并强制页面与接口不缓存。v79 的一次性交接密文由设备 Keystore 保护，成功或失败后删除；应用和日志不保存明文密码。v89源码在回复处理和播放期间继续运行固定Alexa模型，新唤醒先停止本地播放并取消旧会话，旧播放器完全释放后才打开新命令窗口。当前v90进一步实现无需Alexa的直接插话：持续语音通过VAD/能量门槛后立即取消旧回复，释放期间有界暂存原厂PCM，取得新运行所有权后继续上传；播放参考只处理检测副本，不改写上传PCM或宣称AEC效果。两项增量均仅为代码/主机支持，R1功能、播放残留下的误取消和链路状态仍待自动实机验证，见[v90主机记录](docs/2026-09-12-r1-direct-barge-in-host.md)。

当前boot权限环境、专用代理、原厂四麦阵列PCM及DOA传输门槛已实机通过；设备当前运行v28代理和v88 APK。v28已排除v27的固定`invalid=1`和旁路队列竞争，五个实机窗口全部满足schema 2完整性门槛。非专业真人定位、声学和主观体验验收按用户决定停止，不再阻塞开发；历史失败/通过只保留为诊断，不继承为当前结论。实现直接使用原厂四麦、MicArray、DOA、AEC、DSP和校准链；后续持续检查原厂调用、数据、状态、协议、安全、恢复和稳定性，不自行重映射或伪造原厂效果阈值。下一步返回单台核心功能：播放中打断、流式回答、主动播报与计时器、本地闹钟、免打扰、媒体播放、手机蓝牙音箱、安卓/iPhone自助配网、按键/物理静音/灯效、设备名称/区域与系统管理、签名更新与回滚。AEC/DOA/DSP效果及真人唤醒/识别/插话体验标记用户跳过/未验收；R0恢复门槛未取消。以上为待完成交付的功能组，不表示全部尚无代码。
