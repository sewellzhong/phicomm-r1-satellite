...
- 首台设备当前运行 v82 APK与v14 boot代理。v83 已把原厂拓扑“配置双参考/AEC开启”和实机“参考覆盖/AEC消除已证明”拆成独立健康字段；未部署。2026-09-11 经用户明确确认运行 5 秒无刷写调试文件探测：修正首次 Activity 编排缺陷后，v82 普通 APK 入口可复现失败于 `four_mic_open_failed_0`，`/sdcard/unidata` 与 `/data/unidata` 均未生成固定 WAV，卫星最终完整恢复监听。依据免拆分级授权，v84 特权诊断源码已固定于 `68562aaf0d143434dc9fba517bff8bb4cdd70771`，使用 boot 启动参数与单次协议请求双开关，默认保持原厂 debug 文件关闭；主机检查通过，尚未生成、部署或实机验证候选。提交 `c4260c1385b1355abe9aac805a25a7c0af502c8b` 又补齐增量 boot 的候选 overlay 绑定，以及“只替换代理并精确追加单个 init 参数”验证，详见[原厂调试文件探测](docs/2026-09-11-r1-vendor-debug-probe.md)。现场盘点见[硬件能力探测](docs/2026-09-07-r1-hardware-capability.md)。安卓与 iPhone 的 A2DP 配对、播放、断开和重连播放通过。中央键短按和长按已由系统消息桥实机捕获，长按可开启原厂热点；两台手机可同时连接并独立操作配网页面，输入及跨窗口缓存隔离通过。v78 单手机提交后因热点模式下提前写入 Wi-Fi 配置而回退，未通过完整配网。v79 改为使用 Android Keystore 加密一次性交接、恢复 station 后写入，R1 上合成数据往返自检通过；完整手机配网按用户决定留待之后复测。
...
当前boot权限环境、专用代理、原厂四麦阵列PCM及DOA传输门槛已实机通过；v14代理和v82 APK已按门禁部署，无播放、60%和93%受控播放样本已取得。两路运行时输出逐样本相同，DOA在三次固定条件中恒为5度bin。已查明代理原有0参考/false是硬编码的保守验收声明，不是原厂运行时查询；原厂固定配置确实声明双参考和AEC开启，但没有外部PCM注入ABI，也尚未证明本机播放覆盖或消除效果，详见[AEC边界记录](docs/2026-09-11-r1-factory-audio-aec-boundary.md)。当前 v82/v14 无刷写探测已证明普通 APK 路线在打开原厂链时失败且没有落盘；v84 源码已固定，下一步按 boot 双读、仅代理与单个 init 参数变更、单次写入和写后读回门禁部署显式启用的特权诊断代理与原签名 APK，再取得可校准的原厂`4mic/2aec/out`同窗口对照、计算消除量。再以受控物理声源完成四麦独立响应、四方向DOA和DSP质量的R3准入。R0恢复门槛并未取消，只是不再作为首台boot实验的前置。之后补齐单台核心功能：播放中打断、流式回答、主动播报与计时器、本地闹钟、免打扰、媒体播放、手机蓝牙音箱、安卓/iPhone 自助配网、按键/物理静音/灯效、设备名称/区域与系统管理、签名更新与回滚。以上为待完成交付的功能组，不表示全部尚无代码。
