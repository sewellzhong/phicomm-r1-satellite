...
- 当前开发源码为 v70（配网连接确认与重试），首台设备已部署 v69；现场盘点见[硬件能力探测](docs/2026-09-07-r1-hardware-capability.md)，部署与既有验证见[清理报告](docs/2026-09-07-repository-cleanup.md)。安卓与 iPhone 的 A2DP 配对、播放、断开和重连播放通过。v67 复用原厂 NetControl 后，配网灯、热点、网页服务和 Wi-Fi 扫描 API 已通过实测，但原固件首页文件缺失且无提示音。v69 的优化页面和扫描列表实机通过；首次提交只发起连接，R1 未连上临时热点并回退原网络。v70 等待原厂恢复完成后重试目标网络，并以 Android 连接状态决定结果。
