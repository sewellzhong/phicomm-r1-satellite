...
- 已从相同A/B Loader image双份提取805,306,368字节system分区，两份SHA-256一致；固定白名单复核原厂Unisound APK、四麦HAL、圆形/线性MicArray、软件AEC库、配置与AK7755 data2固件。原APK静态调用链确认 `init(1)`、`openAudioIn(2)`、start/read/stop/close及4,800字节读取缓冲，但实际有效通道布局、PCM、DOA和AEC仍须实机证明，详见[system与原厂音频离线审计](docs/2026-09-10-r1-system-audio-offline-audit.md)。
- v80 原厂音频代理已加入真实 3448 原厂库动态后端、固定 v1 Protobuf IPC、Unix socket、`SO_PEERCRED` UID 校验、启动后降权、专用 SELinux 模板、ABI 审计和 APK 生产链证明门槛。2026-09-08 受控真机窗口确认同一原厂进程实际初始化四麦 HAL/MicArray v2.3.0，报告 4 麦、2 路回声参考、AEC 开启，私有音频源打开成功且真人“小讯小讯”到达唤醒事件；六个调试 WAV 仍为 0 帧，故实际 PCM、DOA、AEC消除量和DSP效果保持待验证。详见[原厂链受控运行记录](docs/2026-09-08-r1-original-chain-smoke.md)和[主机开发记录](docs/2026-09-08-r1-factory-audio-agent-host.md)。
