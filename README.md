...
- v28 boot `2499afb281a2c3cfb7c7e47b376501092a94f2dad2aa5f2973717d28ada5ac6f`与v88 APK `b5120e9fb6b2ddcd70d0df592ee9fb249da1fea16a6f18b3a6e82f3b27aa6710`已部署。实时v27 boot双读、仅代理候选、单次写入、复位前完整读回、Android/Enforcing/Wi-Fi/原厂音频预检、APK回读及监听恢复全部通过。v28按调用入口归属诊断窗口，修复“关闭时进入、开启后返回”被误计为`invalid`的竞态；原`trylock`队列改为32槽SPSC环形队列，并把第二生产线程、队列满及三类真实ABI异常独立上报。静默及前、右、后、左五个10秒窗口各取得500帧，所有真实完整性计数均为0，schema 2严格审计通过。正式四方向矩阵仍失败：前305°、后145°、左225°符合拟合零点，而右侧也固定225°，相对期望残差180°，超过45°门槛；R3/R0不变。
...
当前boot权限环境、专用代理、原厂四麦阵列PCM及DOA传输门槛已实机通过；设备当前运行v28代理和v88 APK。v28已排除v27的固定`invalid=1`和旁路队列竞争，五个实机窗口全部满足schema 2完整性门槛；但正式四方向矩阵明确失败，右侧和左侧都报告225°，右侧拟合残差180°。下一步基于这份可复现证据定位原厂DOA的坐标、状态或声源判定边界，不把完整性通过误作方向性通过。已查明代理原有0参考/false是硬编码的保守验收声明，不是原厂运行时查询；原厂固定配置确实声明双参考和AEC开启，但没有外部PCM注入ABI，也尚未证明本机播放覆盖或消除效果，详见[AEC边界记录](docs/2026-09-11-r1-factory-audio-aec-boundary.md)。v20固定文件路线因只得到0字节文件而停止。AEC和DSP质量继续排在方向问题之后。R0恢复门槛并未取消，只是不再作为首台boot实验的前置。再补齐单台核心功能：播放中打断、流式回答、主动播报与计时器、本地闹钟、免打扰、媒体播放、手机蓝牙音箱、安卓/iPhone自助配网、按键/物理静音/灯效、设备名称/区域与系统管理、签名更新与回滚。以上为待完成交付的功能组，不表示全部尚无代码。
