...
当前boot权限环境、专用代理、原厂四麦阵列PCM及DOA传输门槛已实机通过；v14代理和v82 APK已按门禁部署，无播放、60%和93%受控播放样本已取得。两路运行时输出逐样本相同，DOA在三次固定条件中恒为5度bin。已查明代理原有0参考/false是硬编码的保守验收声明，不是原厂运行时查询；原厂固定配置确实声明双参考和AEC开启，但没有外部PCM注入ABI，也尚未证明本机播放覆盖或消除效果，详见[AEC边界记录](docs/2026-09-11-r1-factory-audio-aec-boundary.md)。下一步取得可校准的原厂`4mic/2aec/out`同窗口对照后计算消除量；再以受控物理声源完成四麦独立响应、四方向DOA和DSP质量的R3准入。R0恢复门槛并未取消，只是不再作为首台boot实验的前置。之后补齐单台核心功能：播放中打断、流式回答、主动播报与计时器、本地闹钟、免打扰、媒体播放、手机蓝牙音箱、安卓/iPhone 自助配网、按键/物理静音/灯效、设备名称/区域与系统管理、签名更新与回滚。以上为待完成交付的功能组，不表示全部尚无代码。
