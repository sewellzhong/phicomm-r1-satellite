#!/usr/bin/env python3
"""Probe the privileged 3448 vendor debug-file lifecycle through the v84 boot agent."""
...
EXPECTED_VERSION_CODE = 84
EXPECTED_APK_SHA256 = "f62e058d308495f5165f9e402b023ecfcf8e5b10104197155a0c5c43e789216c"
...
APP_DIAGNOSTIC_ROOT = "/mnt/internal_sd/Android/data/dev.sewellzhong.r1probe/files/diagnostics"
...
def broadcast_succeeded(output):
    return "Broadcast completed: result=0" in output


def validation_output_paths(marker):
    paths = {}
    for key in ("wav_path", "diagnostic_wav_path", "metadata_path"):
        match = re.search(r"(?:^|\s)" + key + r"=([^\s]+)", marker)
        if match is None or match.group(1) == "unavailable":
            raise ProbeError("validation_output_path_missing:" + key)
        remote = match.group(1)
        if not remote.startswith(APP_DIAGNOSTIC_ROOT + "/"):
            raise ProbeError("validation_output_path_outside_diagnostics:" + key)
        name = remote[len(APP_DIAGNOSTIC_ROOT) + 1:]
        if "/" in name or not name.startswith("15"):
            raise ProbeError("validation_output_name_invalid:" + key)
        paths[key] = remote
    return paths


def export_app_capture(device, output, marker):
    directory = output / "app-capture"
    directory.mkdir(mode=0o700)
    exported = {}
    for key, remote in validation_output_paths(marker).items():
        local = directory / remote.rsplit("/", 1)[-1]
        device.pull(remote, local)
        remote_digest = device.remote_sha256(remote)
        local_digest = sha256(local)
        if local_digest != remote_digest:
            raise ProbeError("validation_output_pull_mismatch:" + key)
...
def main(argv=None):
    ...
    result = {
        "device": "r1-sample01",
        "purpose": "privileged_vendor_4mic_2aec_out_lifecycle_and_close_flush_probe",
    ...
    try:
        ...
        # The native stop command updates requested state before the API 22
        # process has necessarily released its proxy client. Force-stop the same
        # package before the explicit protected broadcast so the v84 receiver is
        # the sole capture owner.
        device.shell("am force-stop " + PACKAGE)
        device.adb("logcat", "-c")
        nonce = "privileged-vendor-debug-probe-" + str(time.time_ns())
        broadcast = device.shell(
            "am broadcast -n " + COMPONENT
            + " --es probe_action factory_audio_validate"
            + " --ei duration_seconds 5"
            + " --es sample_id privileged-vendor-debug-lifecycle"
            + " --ez vendor_debug_files true"
            + " --es probe_nonce " + nonce
        )
        result["broadcast"] = broadcast
        if not broadcast_succeeded(broadcast):
            raise ProbeError("vendor_debug_broadcast_failed")
        ...
        result["app_capture"] = export_app_capture(device, output, terminal)
        result["timeline"] = timeline
        time.sleep(0.5)
