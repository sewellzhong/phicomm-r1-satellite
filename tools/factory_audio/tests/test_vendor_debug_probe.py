...
SPEC = importlib.util.spec_from_file_location(
    "vendor_debug_probe", ROOT / "tools/factory_audio/probe-vendor-debug-files.py"
)
probe = importlib.util.module_from_spec(SPEC)
SPEC.loader.exec_module(probe)


class VendorDebugProbeTest(unittest.TestCase):
    ...
    def test_package_identity_requires_data_app_and_version(self):
        version, path = probe.parse_package_identity(
            ...
            "  versionCode=82 targetSdk=22\n"
        )
        self.assertEqual(82, version)
        ...
        with self.assertRaisesRegex(probe.ProbeError, "identity_unavailable"):
            probe.parse_package_identity("versionCode=82")
